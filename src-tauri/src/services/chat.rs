...
/// Results are emitted as `chat_response` Tauri events.
pub async fn execute_command(
    ...
    match cmd {
        ChatCommand::ClaudeCommand { command, options } => {
            ...
            spawn_claude(command, options, app, state).await;
        }
        ...
        ChatCommand::AbortSession {
            ...
            let _ = app.emit("chat_response", ChatResponse::complete(&session_id, &provider, 1, true));
        }
        ChatCommand::CheckSessionStatus {
            ...
            let _ = app.emit("chat_response", ChatResponse {
...
async fn spawn_claude(
    ...
    let mut child = match result {
        ...
        Err(e) => {
            ...
            let _ = app.emit("chat_response", ChatResponse::error(
    ...
    let stdout = match child.stdout.take() {
        ...
        None => {
            ...
            let _ = app.emit("chat_response", ChatResponse::error("No stdout from claude", None, provider));
            schedule_temp_image_cleanup(temp_dir);
            return;
        }
    };
                        ...
                        if let Ok(event) = serde_json::from_str::<Value>(trimmed) {
                            ...
                            if session_id.is_empty() {
                                if let Some(sid) = event.get("session_id").and_then(|v| v.as_str()) {
                                    ...
                                    let _ = app.emit("chat_response", ChatResponse::session_created(&session_id, provider));
                                }
                            }

                            for msg in parsed {
                                let _ = app.emit("chat_response", msg);
                            }
                            ...
                            let _ = app.emit("chat_response", ChatResponse::stream_delta(trimmed, sid, provider));
                        }
                    }
                    Ok(None) => {
    ...
    let _ = app.emit("chat_response", ChatResponse::complete(&session_id, provider, 0, false));
}

// --- Claude Stream Tracker ---------------------------------------------------
                ...
                match block_type.as_str() {
                    "thinking" => {
                        if !tracker.thinking_buffer.is_empty() {
                            msgs.push(ChatResponse {
                                kind: "thinking".to_string(),
                                content: Some(std::mem::take(&mut tracker.thinking_buffer)),
                                session_id: Some(session_id.to_string()),
                                provider: provider.to_string(),
                                ..ChatResponse::empty()
                            });
                    ...
                    "tool_use" => {
                        let input: Value = if !tracker.tool_input_buffer.is_empty() {
                ...
                match line_result {
                    Ok(Some(line)) => {
                        ...
                        if let Ok(event) = serde_json::from_str::<Value>(trimmed) {
                            ...
                            match msg_type {
                                ...
                                "assistant" | "text" => {
                                    let content = event
                                        ...
                                        .unwrap_or("");
                                    if !content.is_empty() {
                                        let _ = app.emit("chat_response", ChatResponse::stream_delta(content, &session_id, provider));
                                    }
                                }
                                "result" => {
                                    if let Some(text) = event.get("result").and_then(|v| v.as_str()) {
                                        let _ = app.emit("chat_response", ChatResponse::stream_delta(text, &session_id, provider));
                                    }
                                }
                                "error" => {
                                    let msg = event
                                    ...
                                    let _ = app.emit("chat_response", ChatResponse::error(msg, Some(&session_id), provider));
                                }
                                _ => {
                                    if let Some(text) = event.get("content").and_then(|v| v.as_str()) {
                                        let _ = app.emit("chat_response", ChatResponse::stream_delta(text, &session_id, provider));
                            ...
                            if !session_id.is_empty() {
                                let _ = app.emit("chat_response", ChatResponse::stream_delta(trimmed, &session_id, provider));
                            }
                        }
                    }
                    Ok(None) => {
...
async fn spawn_codex(
    ...
    let stdout = match child.stdout.take() {
        ...
        None => {
            error!(provider, "No stdout from CLI process");
            let _ = app.emit("chat_response", ChatResponse::error("No stdout from codex", None, provider));
            return;
        }
    };
...
async fn spawn_gemini(
    ...
    let stdout = match child.stdout.take() {
        ...
        None => {
            error!(provider, "No stdout from CLI process");
            let _ = app.emit("chat_response", ChatResponse::error("No stdout from gemini", None, provider));
            return;
        }
    };
